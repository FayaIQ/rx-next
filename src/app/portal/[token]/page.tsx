    ...
    <div className="min-h-screen bg-gradient-to-b from-teal-50 to-white px-4 py-8">
      <div className="mx-auto max-w-lg space-y-6">
        <header className="text-center">
          ...
          <p className="mt-1 text-sm text-slate-600">بوابة المريض</p>
        </header>
        ...
        <section className="rounded-2xl border bg-white p-4 shadow-sm">
          ...
          {data.appointments.length === 0 ? (
            <p className="mt-2 text-sm text-slate-500">لا توجد مواعيد قادمة.</p>
          ) : (
                ...
                (a: {
                  ...
                  <li key={a.id} className="rounded-lg border px-3 py-2">
                    <p className="font-medium">
                      {a.date} ·{" "}
                      {new Date(a.time).toLocaleTimeString("ar-SY", {
                        hour: "2-digit",
                        minute: "2-digit",
                      })}
                    </p>
        ...
        {data.treatmentSessions.length > 0 ? (
          <section className="rounded-2xl border bg-white p-4 shadow-sm">
            <h2 className="font-semibold text-slate-900">جلسات العلاج القادمة</h2>
            <ul className="mt-3 space-y-2 text-sm">
              {data.treatmentSessions.map(
                (s: {
                  ...
                  <li key={s.id} className="rounded-lg border px-3 py-2">
                    {s.label} — سن {s.toothFdi} · جلسة {s.sessionNumber}
                    {s.date ? ` · ${s.date}` : ""}
                  </li>
                )
              )}
        ...
        {data.clinicPhone ? (
          <p className="text-center text-xs text-slate-500">
            للاستفسار: <span dir="ltr">{data.clinicPhone}</span>
          </p>
        ) : null}
      </div>
    </div>
